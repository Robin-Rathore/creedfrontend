...
export const useUpdateUserRole = () => {
  ...
  return useMutation({
    mutationFn: ({
      id,
      role,
    }: { id: string; role: string }): Promise<{ success: boolean; message: string; data: User }> =>
...
export const useDeleteUser = () => {
  ...
  return useMutation({
    mutationFn: (id: string): Promise<{ success: boolean; message: string }> => apiClient.delete(`/admin/users/${id}`),
    onSuccess: (response) => {
      queryClient.invalidateQueries({ queryKey: queryKeys.admin.users() })
      toast.success(response.message)
    },
