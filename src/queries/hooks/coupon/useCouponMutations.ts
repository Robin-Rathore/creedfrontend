...
export const useCreateCoupon = () => {
  ...
  return useMutation({
    mutationFn: (data: CreateCouponRequest): Promise<{ success: boolean; message: string; data: Coupon }> =>
...
export const useUpdateCoupon = () => {
  ...
  return useMutation({
    mutationFn: ({
      id,
      ...data
    }: CreateCouponRequest & { id: string }): Promise<{ success: boolean; message: string; data: Coupon }> =>
...
export const useDeleteCoupon = () => {
  ...
  return useMutation({
    mutationFn: (id: string): Promise<{ success: boolean; message: string }> => apiClient.delete(`/coupons/${id}`),
    onSuccess: (response) => {
      queryClient.invalidateQueries({ queryKey: queryKeys.coupons.all() })
      queryClient.invalidateQueries({ queryKey: queryKeys.coupons.active })
      toast.success(response.message)
